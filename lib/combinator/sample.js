...
function sample(f, sampler, ...streams) {
	return new Stream(new Sampler(f, sampler.source, pluck(streams, 'source')));
}
...
function sampleWith(sampler, stream) {
	return new Stream(new Sampler(identity, sampler.source, [stream.source]));
}
...
class SampleSink {
	...
	_notify() {
		if (!this.active) {
			this.active = this.sinks.every((hold) => hold.hasValue);
		}
	}
